...
//process the http post data by action
	if ($action != '' && is_array($teachers) && @sizeof($teachers) != 0) {
		...
		header('Location: daf_teachers.php'.($search != '' ? '?search='.urlencode($search) : null));
		exit;
	}
...
//get total extension count for domain
    // if (is_numeric($_SESSION['limit']['extensions']['numeric'])) {
        $sql = "select count(*) from v_chazara_daf_teachers ";
...
//get total extension count
    $sql = "select count(*) from v_chazara_daf_teachers where true ";
...
//get the extensions
    ...
    if ($order_by == '' || $order_by == 'chazara_daf_teacher_uuid') {
            $sql .= 'order by name asc'.$order; //function in app_defaults.php
    }
    else {
...
// print_r($_SESSION['theme']);exit;
    ...
    if (permission_exists('chazara_teacher_edit') && (!is_numeric($_SESSION['limit']['teachers']['numeric']) || $total_extensions < $_SESSION['limit']['teachers']['numeric'])) {
        echo button::create(['type'=>'button','label'=>$text['button-add'],'icon'=>$_SESSION['theme']['button_icon_add'],'id'=>'btn_add','style'=>$margin_left,'link'=>'daf_teachers_edit.php']);
        unset($margin_left);
    }
...
if (is_array($teachers) && @sizeof($teachers) != 0) {
    ...
    foreach($teachers as $row) {
        if (permission_exists('chazara_teacher_edit')) {
            $list_row_url = "daf_teachers_edit.php?id=".urlencode($row['chazara_daf_teacher_uuid']).(is_numeric($page) ? '&page='.urlencode($page) : null);
        }
        ...
        if (permission_exists('chazara_teacher_delete') || permission_exists('chazara_teacher_delete')) {
            ...
            echo "		<input type='hidden' name='teachers[$x][uuid]' value='".escape($row['chazara_daf_teacher_uuid'])."' />\n";
            echo "	</td>\n";
        }
        if ($_GET['show'] == "all" && permission_exists('chazara_recording_all')) {
        ...
        echo "	<td>".escape($row['name_recording_path'])."&nbsp;</td>\n";
        echo "</tr>\n";
        $x++;
    }
}
